...
def deserealized_regressor(json_model):
    import numpy
    ...
    set_param(datrics_model, json_model, "model_train_parameters", 'model_parameters')
    set_param(datrics_model, json_model, "regularization", 'model_parameters')
    ...
    dtypes = {}
    for key in json_model["dtypes"].keys():
        dt = json_model["dtypes"].get(key)
        dtypes[key] = numpy.dtype(dt)

    datrics_model['model_parameters']['data_types'] = dtypes
